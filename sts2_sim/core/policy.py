...
def estimate_attack_damage(card: "STS2Card", player, target: "MonsterModel") -> int:
    ...
    def choose(self, combat: "CombatState") -> Optional[Tuple["STS2Card", Optional["MonsterModel"]]]:
        ...
        for target in sorted(enemies, key=lambda m: m.current_hp):
            ...
            killers = [c for c in attacks
                       if estimate_attack_damage(c, player, target) >= effective_hp]
            if killers:
                # 최소 비용으로 처치
                best = min(killers, key=lambda c: (c.cost, estimate_attack_damage(c, player, target)))
                return (best, target)

        # 2) 공격 vs 방어: 에너지당 가치 비교
        ...
        def attack_score(c) -> float:
            score = estimate_attack_damage(c, player, target)
            if target.get_power_amount("vulnerable") == 0:
                score += _VULN_SETUP_BONUS.get(c.card_id, 0)
            return score / max(1, c.cost)
        ...
        best_blocker = None
        block_value = -1.0
        blockers = [c for c in skills if _card_value(_BLOCK_VALUE, c) > 0]
        if blockers:
            def hp_saved(c):
                return min(_card_value(_BLOCK_VALUE, c), max(0, incoming - player.block))
            best_blocker = max(blockers, key=lambda c: hp_saved(c) / max(1, c.cost))
            block_value = hp_saved(best_blocker) / max(1, best_blocker.cost)
        ...
        # 3) 유틸 스킬 (오브/스타 등)
        utility = [c for c in skills if _card_value(_BLOCK_VALUE, c) == 0]
        if utility:
